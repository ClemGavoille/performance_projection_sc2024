...
if [[ -n $1 ]]; then
	...
	if [[ "$MPI_DEFAULT" -eq 1 ]]; then  # Default OpenMP environment
		N_PROCESSES=1
	fi
	appname=$(basename $1)
	...
	mpiexec -n ${N_PROCESSES} $DYNAMORIO_DIR/bin64/drrun -t drcachesim -raw_compress gzip -coherence -offline -L0I_filter -L0I_size 0 -max_trace_size ${TRACE_SIZE}M -outdir $TMP_TRACE_DIR/$appname/ -- $@ 
