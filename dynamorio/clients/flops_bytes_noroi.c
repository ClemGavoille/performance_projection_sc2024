...
FILE *logfile;
char filename[sizeof("flops_bytes.1000000.log")];
...

{
	...
	if (counter->fp_ins != 0 || counter->bytes != 0 || counter->int_ins != 0)//|| counter->sve_instr != 0)
		...
		bool for_trace, bool translating,void *user_data)
{
	...
	if (instr_fetch != NULL) {
		...
		if (instr_operands == NULL ||
				(!instr_reads_memory(instr_operands) && !instr_writes_memory(instr_operands) && !is_intop(instr_operands) && !is_flop(instr_operands)))
			return DR_EMIT_DEFAULT;
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
	...
	sprintf(filename,"flops_bytes.%05d.log",dr_get_process_id());
	printf("%s\n",filename);
	logfile = fopen(filename,"w");
	fprintf(logfile,"Run,TID,Flops,Bytes,FPINS,Intops,INTINS\n");
}
